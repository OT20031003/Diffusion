...
"""
これはだめ。毎回レイヤが作成されるためパラメータ更新されない
def Upsample(x):
    # input : x tensor型
    dim  = len(x[0])
    fn = torch.nn.Upsample(scale_factor=2, mode='nearest')
    fn2 = torch.nn.Conv2d(in_channels=dim, out_channels=dim, kernel_size=3, padding=1)
    return fn2(fn(x))
"""
